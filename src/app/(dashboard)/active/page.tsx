      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        {!connected && users.length === 0 ? (
          ...
          <div className="text-center py-20 text-slate-400">
            ...
            <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">No active sessions</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
                  ...
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">User</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">IP Address</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide hidden md:table-cell">MAC Address</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Uptime</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide hidden sm:table-cell">Time Left</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide hidden lg:table-cell">Data In</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide hidden lg:table-cell">Data Out</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide hidden xl:table-cell">Server</th>
                  <th className="text-right py-3 px-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide"></th>
                </tr>
              </thead>
              <tbody>
                {users.map((u, i) => (
                  <tr
                    key={`${u.id}-${u.user}`}
                    className={`border-b border-slate-100 dark:border-slate-700/50 hover:bg-indigo-50/40 dark:hover:bg-indigo-900/20 transition-colors ${
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-slate-600 dark:text-slate-300 tabular-nums text-xs"
                        style={{ fontFamily: 'var(--font-mono)' }}>
                      {u.address}
                    </td>
                    ...
                    <td className="py-3 px-4 text-slate-700 dark:text-slate-200 font-medium tabular-nums text-xs">{u.uptime}</td>
                    ...
                    <td className="py-3 px-4 text-slate-500 dark:text-slate-400 tabular-nums text-xs hidden lg:table-cell">
                      {formatBytes(u.bytesIn)}
                    </td>
                    <td className="py-3 px-4 text-slate-500 dark:text-slate-400 tabular-nums text-xs hidden lg:table-cell">
                      {formatBytes(u.bytesOut)}
                    </td>
                    <td className="py-3 px-4 text-slate-400 dark:text-slate-500 dark:text-slate-400 text-xs hidden xl:table-cell">{u.server}</td>
